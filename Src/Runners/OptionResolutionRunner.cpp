...
#include "Src/Domain/Tests/OptionTest.h"
...
RunnerConfig OptionResolutionRunner::buildConfig()
{
    ...
    worker->SetTask(task);
    worker->setMode(OptionTest::Mode::Resolution);
...
void OptionResolutionRunner::wireSpecificSignals(Test& base) {
    ...
    connect(&t, &OptionTest::GetPoints,
            owner, &Program::receivedPoints_resolutionTest,
            Qt::BlockingQueuedConnection);

    connect(&t, &OptionTest::Results,
            owner, &Program::results_resolutionTest,
            Qt::QueuedConnection);
}
