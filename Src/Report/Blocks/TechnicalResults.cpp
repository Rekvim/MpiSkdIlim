...
namespace Report::Blocks {
    ...
    void TechnicalResults::build(Writer& writer, const Context& ctx)
    {
        ...
        if (!main) {
            qWarning() << "[Blocks::TechnicalResults] отсутствует ctx.telemetry.testMain;";
        }
        ...
        if (v.dinamicErrorRecomend == "Без позиционера" && main) {
        ...
        if (t.testStroke) {
            ...
            qWarning() << "[Blocks::TechnicalResults] отсутствует ctx.telemetry.testStroke";
        }
    }
}