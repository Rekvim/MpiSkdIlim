...
static QString joinNumbers(const QVector<double>& values)
{
    QStringList list;

    for (int i = 0; i < values.size(); ++i) {
        list << "шаг " + QString::number(i + 1) + " — " + QString::number(values[i], 'f', 1) + "%";
    }
    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->tableWidget_stepResults->setHorizontalHeaderLabels(
        { "T86", tr("Перерегулирование") }
        );

    ui->tableWidget_stepResults->horizontalHeader()->setStyleSheet(
        "QHeaderView::section:first { background-color:yellow; }"
        "QHeaderView::section:last  { background-color:#fd7d13; }"
        );
    ...
    buildOptionTable(
        ui->tableWidget_response,
        m_telemetryStore.responseResults,
        OptionTableKind::Response
    );

    buildOptionTable(
        ui->tableWidget_resolution,
...
void MainWindow::setStepTestResults(const QVector<StepTest::TestResult> &results, quint32 T_value)
{
    ...
    ui->tableWidget_stepResults->setHorizontalHeaderItem(0, item1);
    ui->tableWidget_stepResults->setHorizontalHeaderItem(1, item2);
    ui->tableWidget_stepResults->resizeColumnsToContents();
...
void MainWindow::onResolutionTestParametersRequested(OtherTestSettings::TestParameters &parameters)
{
    ...
    if (m_resolutionTestSettings->exec() == QDialog::Accepted) {
        ...
        ui->label_resolution_pointsPreview->setText("Выбранное положение хода — " +  QString::number(parameters.points[0]) + "%");
        ui->label_resolution_stepsPreview->setText("Выбранный ступенчатый шаг: " + joinNumbers(parameters.steps));

        ui->label_resolution_pointsPreview->setStyleSheet(
            "border: none;"
            "padding-bottom: 2px;"
            "border-bottom: 2px solid #fd7d13;"
        );

        ui->label_resolution_stepsPreview->setStyleSheet(
            "border: none;"
            "padding-bottom: 2px;"
...
void MainWindow::onResponseTestParametersRequested(OtherTestSettings::TestParameters &parameters)
{
    ...
    if (m_responseTestSettings->exec() == QDialog::Accepted) {
        ...
        ui->label_response_pointsPreview->setText("Выбранное положение хода — " + QString::number(parameters.points[0]) + "%");
        ui->label_response_stepsPreview->setText("Выбранный шаг: " + joinNumbers(parameters.steps));

        ui->label_response_pointsPreview->setStyleSheet(
            "border: none;"
            "padding-bottom: 2px;"
            "border-bottom: 2px solid #fd7d13;"
        );

        ui->label_response_stepsPreview->setStyleSheet(
            "border: none;"
            "padding-bottom: 2px;"
            "border-bottom: 2px solid green;"
        );
...
void MainWindow::initCharts()
{
    ...
    m_charts[Charts::Trend]->addSeries(0, tr("Задание"), taskColor);
    ...
    m_charts[Charts::Stroke]->addSeries(0, tr("Задание"), taskColor);
    ...
    m_charts[Charts::Task]->addSeries(1, tr("Задание"), taskColor);
    ...
    m_charts[Charts::Pressure]->addSeries(0, tr("Линейная регрессия"), taskColor);
    ...
    m_charts[Charts::Resolution]->addSeries(0, tr("Задание"), taskColor);
    ...
    m_charts[Charts::Response]->addSeries(0, tr("Задание"), taskColor);
    ...
    m_charts[Charts::Step]->addSeries(0, tr("Задание"), taskColor);
    ...
    connect(m_program, &Program::getPoints_responseTest,
            this, &MainWindow::onStepTestPointsRequested,
            Qt::BlockingQueuedConnection);

    connect(m_program, &Program::getPoints_resolutionTest,
            this, &MainWindow::onStepTestPointsRequested,
            Qt::BlockingQueuedConnection);
}
