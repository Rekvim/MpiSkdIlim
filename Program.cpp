...
void Program::receivedPoints_responseTest(QVector<QVector<QPointF>> &points)
{
    emit getPoints_responseTest(points, Charts::Response);
}

void Program::results_resolutionTest(const OptionTest::OptionResults &results)
{
    m_telemetryStore.resolutionResults.upward.clear();
    m_telemetryStore.resolutionResults.downward.clear();

    m_telemetryStore.resolutionResults.baseTask   = 0.0;
    m_telemetryStore.resolutionResults.baseActual = 0.0;
    m_telemetryStore.resolutionResults.baseActual_two = 0.0;

    if (std::isfinite(results.baseTask))
        m_telemetryStore.resolutionResults.baseTask = results.baseTask;

    if (std::isfinite(results.baseActual))
        m_telemetryStore.resolutionResults.baseActual = results.baseActual;

    if (std::isfinite(results.baseActual_two))
        m_telemetryStore.resolutionResults.baseActual_two = results.baseActual_two;

    for (const auto& r : results.upward)
    {
        if (!std::isfinite(r.selectedPercent) ||
            !std::isfinite(r.actualPercent) ||
            !std::isfinite(r.stepErrorPercent))
            continue;

        OptionStepRecord rec;
        rec.selectedPercent  = r.selectedPercent;
        rec.actualPercent    = r.actualPercent;
        rec.stepErrorPercent = r.stepErrorPercent;
        m_telemetryStore.resolutionResults.upward.push_back(rec);
    }

    for (const auto& r : results.downward)
    {
        if (!std::isfinite(r.selectedPercent) ||
            !std::isfinite(r.actualPercent) ||
            !std::isfinite(r.stepErrorPercent))
        ...
        rec.actualPercent    = r.actualPercent;
        rec.stepErrorPercent = r.stepErrorPercent;
        m_telemetryStore.resolutionResults.downward.push_back(rec);
    }

    emit telemetryUpdated(m_telemetryStore);
}

void Program::results_responseTest(const OptionTest::OptionResults &results)
{
    m_telemetryStore.responseResults.upward.clear();
    m_telemetryStore.responseResults.downward.clear();

    m_telemetryStore.responseResults.baseTask   = 0.0;
    m_telemetryStore.responseResults.baseActual = 0.0;
    m_telemetryStore.responseResults.baseActual_two = 0.0;

    if (std::isfinite(results.baseTask))
        m_telemetryStore.responseResults.baseTask = results.baseTask;

    if (std::isfinite(results.baseActual))
        m_telemetryStore.responseResults.baseActual = results.baseActual;

    if (std::isfinite(results.baseActual_two))
        m_telemetryStore.responseResults.baseActual_two = results.baseActual_two;
    ...
    for (const auto& r : results.upward)
    {
        ...
        rec.actualPercent    = r.actualPercent;
        rec.stepErrorPercent = r.stepErrorPercent;
        m_telemetryStore.responseResults.upward.push_back(rec);
    }
    ...
    for (const auto& r : results.downward)
    {
        ...
        rec.actualPercent    = r.actualPercent;
        rec.stepErrorPercent = r.stepErrorPercent;
        m_telemetryStore.responseResults.downward.push_back(rec);
    }

    emit telemetryUpdated(m_telemetryStore);
}

void Program::receivedPoints_resolutionTest(QVector<QVector<QPointF>> &points)
{
    emit getPoints_resolutionTest(points, Charts::Resolution);
}
